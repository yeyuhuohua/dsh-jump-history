window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function JumpHistoryAction(props) {
      var ctx = props.ctx;
      // DSH 0.1.2：chat 读取通道从 useSession(s => s.chat) 迁移到独立的 useChat hook
      ...
      // 优先 useChat（DSH 0.1.2 新通道）；兼容旧版回退 useSession(s => s.chat)
      var chatSlice = null;
      if (useChat !== null) {
        chatSlice = useChat(function (s) { return s.legacy; });
      } else if (useSession !== null) {
        var legacy = useSession(function (s) { return s.chat !== undefined ? s.chat.legacy : undefined; });
        chatSlice = legacy !== null && legacy !== undefined ? legacy : null;
      }
      var allRows = chatSlice !== null
        ? collectUserRows(chatSlice.nodes)
        : [];
